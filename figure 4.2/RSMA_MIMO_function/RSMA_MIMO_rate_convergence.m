...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [R_sum]=RSMA_MIMO_rate_convergence(H,Nr,SNR,sigma,weight,tolerance,max_count,alpha)
        ...
        Pri_precoder = sqrt(a_k/Num_U_Nr) .* direction; % private precoder design
        % should have size of  Nt*Num_User for each user
        % make sure norm(direction(:,i)) is 1.

    
        [U,~,~] = svd(H);
        u = U(:,1:N_common); % choose the leftest singular vector based on the number of transmit common stream
        Common_precoder = sqrt(a_c/N_common).*u; % common precoder design
