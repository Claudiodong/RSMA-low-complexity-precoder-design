...
function [Rate_p_k,Pri_precoder]=SDMA_MIMO_rate(H,Nr,max_count,tolerance,sigma,SNR,weight)
    ...
    for i = 1:Num_U_Nr
         direction(:,i) = H(i,:)'/norm(H(i,:)); % ensure norm(direction(:,i)) is 1
    end
    % Initilised the precoder (MRT-SVD)
    Pri_precoder = sqrt(SNR/(Num_U_Nr)) * direction; % private precoder design
